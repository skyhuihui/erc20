...
contract Bancor is BancorFormula {
    using SafeMath for uint256;
	...
	function () payable external{
	    if (msg.sender != owner){
	        ...
	        transferAmount = BancorFormula.calculatePurchaseReturn(supply, blance, cw, msg.value);
	        if (balanceOf(address(this)) > transferAmount){
    ...
    constructor (uint256 _totalSupply, string memory _symbol, string memory _name) public {
		decimals = 18;
		symbol = _symbol;
		name = _name;
		owner = msg.sender;
    ...
    function transfer(address _to, uint256 _amount) onlyPayloadSize(2 * 32) public returns (bool success) {
        ...
        if (_to == address(this)){
            // 卖出代币  
            uint256 transferEth;
	        transferEth = BancorFormula.calculateSaleReturn(supply, blance, cw, _amount);
	        if (address(this).balance >= transferEth ){
	            address(msg.sender).transfer(transferEth);
	            blance -= transferEth;
	            supply -= _amount;
	        }
        }
	...
	function withdraw() public {
        ...
        address(msg.sender).transfer(etherBalance);
    }
